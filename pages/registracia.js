...
export default function SignUpPage() {
  ...
  const handleTermsAgreedChange = (isChecked) => {
    setForm({ ...form, termsAgreed: isChecked });
  };
  ...
  async function handleSubmit(e) {
    ...
    if (emptyFields.length > 0) {
      return setError("Prosím, vyplňte všetky povinné polia.");
    }

    if (!form.termsAgreed) {
      return setError("Musíte súhlasiť s podmienkami používania.");
    }

    if (form.password.length < 8) {
      return setError("Heslo musí obsahovať minimálne 8 znakov.");
    }
    if (!/[0-9]/.test(form.password)) {
      return setError("Heslo musí obsahovať aspoň jedno číslo.");
    }
    if (!/[A-Z]/.test(form.password)) {
      return setError("Heslo musí obsahovať aspoň jedno veľké písmeno.");
    }
            ...
            <div className="lg:col-span-3">
              <div className="backdrop-blur-md bg-zinc-900/70 rounded-3xl p-8 md:p-10 border border-zinc-700/50 shadow-xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {error && (
                    <p className="text-red-400 bg-red-900/40 border border-red-500 rounded-md p-2 text-sm text-center">
                      {error}
                    </p>
                  )}
